...
## 2026-03-02 - M5 iOS Distribution/Testing Flow (Firebase) 完了
...
- Milestone: `M5`
- Summary of changes:
  - iOS 配布/検証 Runbook `docs/ios-support/distribution-testing-flow.md` を追加
  - iOS Firebase 配布 workflow `.github/workflows/distribute-ios-firebase.yml` を追加
  - 手動実行（`workflow_dispatch`）/内部テスター配布/`main` 最新コミット限定を運用ルール化
  - 受け入れテスト観点（配布成功 + インストール確認 + 最小動作）と責任分界（開発チーム一元管理）を明文化
  - `README.md` に iOS 配布導線を追加し、`roadmap.md` の M5 を completed に更新
- Verification commands + results:
  - `ruby -e "require 'yaml'; YAML.load_file('.github/workflows/distribute-ios-firebase.yml'); puts 'workflow yaml ok'"`
  - 結果: `workflow yaml ok`
  - `rg -n \"M5|distribution-testing-flow|Distribute iOS \\(Firebase\\)\" README.md docs/ios-support/roadmap.md docs/ios-support/distribution-testing-flow.md .github/workflows/distribute-ios-firebase.yml`
  - 結果: 期待した定義/導線/完了状態を確認
- Known gaps / next action:
  - iOS 署名素材（証明書/プロビジョニング）と Firebase iOS app 設定の実環境投入が必要
  - macOS 上で `Distribute iOS (Firebase)` 実行実績を取り、内部テスター端末でインストール確認を実施
- PR link:
  - N/A (to be created)
