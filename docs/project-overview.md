# Org Clock Project Overview

このドキュメントは、Org Clock リポジトリの現行の全体概要をまとめた内部・開発向けの総合案内です。

## Original README Snapshot

以下は、ルート `README.md` から移設した内容です。

# Org Clock

Android 端末と Desktop OS 上で org ファイルの見出しに対して clock 記録を行うアプリです。
...
Desktop MVP は Compose Desktop ベースのクロスプラットフォーム検証ホストです。Android と同じ org repository / clock domain を段階的に共有していく前段として、Windows / Linux 上で起動・配布・手動検証できる状態を維持します。

前提:

- Windows 11 または Linux x86_64 環境
- JDK 17
...
- OS 別 installer/distribution: `desktopApp/build/compose/binaries/main/`
  - Windows: `.msi`
  - Linux: `.deb` / `.AppImage`
...
- Android と同等の詳細 editing / notification UX

Desktop ではまず「起動できること」「将来の共有 UI / domain の受け皿になること」「配布物を生成できること」を優先しています。
...
- Desktop MVP の土台として `:desktopApp` を追加しています。ローカル Windows / Linux での起動は `./gradlew runDesktop` または `./gradlew :desktopApp:run` を使用します。
