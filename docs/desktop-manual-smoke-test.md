...
GitHub Actionsなど非対話Windows環境では、GUIアプリを起動せず、
通常版MSIのインストール、exe配置、スタートメニュー・デスクトップショートカット、
アンインストールだけを確認する。
...
SQLite、org読み書き、clock操作、テンプレート読込は `:desktopApp:test` で検証する。
インストール済みGUIアプリの実起動は、対話可能なWindows端末で通常のスモークテストを実行する。
